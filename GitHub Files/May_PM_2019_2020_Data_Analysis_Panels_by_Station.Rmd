---
title: "PM2.5 Analysis for May 2019-2020 BAU v.s. COVID"
...
# Load data from master files. Since the relevant data 
# was sent in two separate files, we will need to join 
# them togehter for analysis. 
...
Edit dataframe: 
```{r message = FALSE, warning = FALSE}
# We're left with dataframes containing hourly PM values.
...
#Fitler months and years. 
PM <- PM %>%
  filter(Year > 18,
         Month == 5)

# Remove values in 2020 that do not allow for 
# table joining.
...
# Join tables to make the master PM table. 
PM <- PM %>%
  inner_join(pm_1920)

# Re-format the dataframe to make it more conductive to vizualisation and stats. 
PM <- PM %>%
...
Diurnal weekend averages by year:
```{r}
# Weekend summary dataframe.
